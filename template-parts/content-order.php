<?php
/**
 * Template part for displaying page order in page-order.php
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package spiver
 */
?>
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <div class="entry-content">
        <div id="c_data"></div>
        <?php if(!empty( $_SESSION['cart'] ) || $_SESSION['cart'] != null):?>
        <table class="ui celled striped table">
            <thead>
                <tr class="order_header_top">
                    <th class="center aligned">
                        <?php _e( 'ID', 'spiver' )?>
                    </th>
                    <th class="center aligned"><?php _e( 'Product name', 'spiver' )?> </th>
                    <th class="center aligned"><?php _e( 'Description', 'spiver' );?></th>
                    <th class="center aligned"><?php _e( 'Action', 'spiver' );?></th>
                </tr>
            </thead>
            <tbody id="orderTable">
                <?php
					$order = $_SESSION['cart'];
					foreach($order as $key => $arrays): ?>
                <?php $json_array = json_decode($arrays, true);?>
                <tr class="data_cart">
                    <td class="data_id">
                        <?php echo $json_array['id']?>
                    </td>
                    <td class="data_title"><?php echo $json_array['title']?></td>
                    <td class="data_description"><?php echo $json_array['Description']?></td>
                    <td class="center aligned">
                        <a href="#" class="delBtn">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0)">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        d="M3.125 1.25C2.79348 1.25 2.47554 1.3817 2.24112 1.61612C2.0067 1.85054 1.875 2.16848 1.875 2.5V3.75C1.875 4.08152 2.0067 4.39946 2.24112 4.63388C2.47554 4.8683 2.79348 5 3.125 5H3.75V16.25C3.75 16.913 4.01339 17.5489 4.48223 18.0178C4.95107 18.4866 5.58696 18.75 6.25 18.75H13.75C14.413 18.75 15.0489 18.4866 15.5178 18.0178C15.9866 17.5489 16.25 16.913 16.25 16.25V5H16.875C17.2065 5 17.5245 4.8683 17.7589 4.63388C17.9933 4.39946 18.125 4.08152 18.125 3.75V2.5C18.125 2.16848 17.9933 1.85054 17.7589 1.61612C17.5245 1.3817 17.2065 1.25 16.875 1.25H12.5C12.5 0.918479 12.3683 0.600537 12.1339 0.366117C11.8995 0.131696 11.5815 0 11.25 0L8.75 0C8.41848 0 8.10054 0.131696 7.86612 0.366117C7.6317 0.600537 7.5 0.918479 7.5 1.25H3.125ZM6.875 6.25C7.04076 6.25 7.19973 6.31585 7.31694 6.43306C7.43415 6.55027 7.5 6.70924 7.5 6.875V15.625C7.5 15.7908 7.43415 15.9497 7.31694 16.0669C7.19973 16.1842 7.04076 16.25 6.875 16.25C6.70924 16.25 6.55027 16.1842 6.43306 16.0669C6.31585 15.9497 6.25 15.7908 6.25 15.625V6.875C6.25 6.70924 6.31585 6.55027 6.43306 6.43306C6.55027 6.31585 6.70924 6.25 6.875 6.25V6.25ZM10 6.25C10.1658 6.25 10.3247 6.31585 10.4419 6.43306C10.5592 6.55027 10.625 6.70924 10.625 6.875V15.625C10.625 15.7908 10.5592 15.9497 10.4419 16.0669C10.3247 16.1842 10.1658 16.25 10 16.25C9.83424 16.25 9.67527 16.1842 9.55806 16.0669C9.44085 15.9497 9.375 15.7908 9.375 15.625V6.875C9.375 6.70924 9.44085 6.55027 9.55806 6.43306C9.67527 6.31585 9.83424 6.25 10 6.25V6.25ZM13.75 6.875C13.75 6.70924 13.6842 6.55027 13.5669 6.43306C13.4497 6.31585 13.2908 6.25 13.125 6.25C12.9592 6.25 12.8003 6.31585 12.6831 6.43306C12.5658 6.55027 12.5 6.70924 12.5 6.875V15.625C12.5 15.7908 12.5658 15.9497 12.6831 16.0669C12.8003 16.1842 12.9592 16.25 13.125 16.25C13.2908 16.25 13.4497 16.1842 13.5669 16.0669C13.6842 15.9497 13.75 15.7908 13.75 15.625V6.875Z"
                                        fill="#FF0000" />
                                </g>
                                <defs>
                                    <clipPath id="clip0">
                                        <rect width="20" height="20" fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <?php else:?>
        <div class="ui floating orange message"><?php _e( 'There is no orders yet!', 'spiver' );?></div>
        <?php endif;?>
    </div><!-- .entry-content -->
</article><!-- #post-<?php the_ID(); ?> -->